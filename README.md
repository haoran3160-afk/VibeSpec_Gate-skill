...
> VibeSpec Gate 提供辅助判断，不能替代专业渗透测试、安全认证、法律意见或合规审计。
...
安装完成后重启 Codex，再新建一个 Agent 任务。`$skill-installer` 默认安装到 `$CODEX_HOME/skills/vibespec-gate`（通常是 `$HOME/.codex/skills/vibespec-gate`）。手动安装也可使用用户级 `$HOME/.agents/skills/vibespec-gate` 或仓库级 `.agents/skills/vibespec-gate`。
...
- 安装包完整性和受支持平台由 CI 持续校验。
- 审查结论只覆盖当前任务能够读取的证据；缺失、不可读或截断的证据会返回 `REVIEW`。
- “只读审查”仍取决于 Agent 环境的权限和可审计记录；本 Skill 不能独立证明文件系统从未被修改。
- 自动化检查不能证明没有漏洞，也不能替代专业安全评估。
