...
def run_review(
    ...
    selected, skipped = select_findings(findings, include_p2)
    ...
    data = summary(profile, findings, packets, verdicts, skipped, queue, candidates)
    decision_output = agent_review_decisions_json(data, decisions)
    llm_packet = build_llm_review_packet(profile, findings, packets, verdicts)
